...
public class ReplyButtonListener implements ActionListener {
	...
	public ReplyButtonListener(MainPanel mainPanel, TweetBox tweetBox,
			Tweet tweet) {
		this.tweet = tweet;
		...
		while (token.hasMoreTokens()) {
			...
			if (ifMention(currentToken)) {
				...
				if (ifCurrentUser(currentToken))
					continue;
				String plainAlphaNumbericReprentation = currentToken
	...
	private boolean ifCurrentUser(String currentToken) {
		return (currentToken.substring(1)).equals(mainPanel
